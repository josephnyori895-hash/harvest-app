...
export default async function givingRoutes(app) {
  app.post('/api/giving/mpesa/stkpush', { preHandler: [requireMember] }, async (req, reply) => {
    try {
      ...
      await pool.query(
        `UPDATE giving_transactions SET merchant_request_id=$2, checkout_request_id=$3, metadata=metadata || $4::jsonb WHERE id=$1`,
        [pending.rows[0].id, body.MerchantRequestID, body.CheckoutRequestID, JSON.stringify({ customerMessage: body.CustomerMessage })],
      )
  ...
  app.get('/api/giving/mine', { preHandler: [requireMember] }, async (req, reply) => {
    const { rows } = await pool.query(
      `SELECT id, amount_kes, purpose, provider, status, receipt_number, created_at, completed_at
       FROM giving_transactions WHERE user_id=$1 ORDER BY created_at DESC LIMIT 100`,
      [req.user.id],
    )
      ...
      if (code === '0') {
        ...
        await pool.query(
          `UPDATE giving_transactions
           SET status='completed', receipt_number=$2, provider_result_code=$3, provider_result_description=$4,
               metadata=metadata || $5::jsonb, completed_at=now()
           WHERE id=$1`,
          [tx.id, receipt || null, code, String(result.ResultDesc || ''), JSON.stringify(metadata)],
        )
      } else {
        await pool.query(
          `UPDATE giving_transactions SET status='failed', provider_result_code=$2, provider_result_description=$3 WHERE id=$1`,
          [tx.id, code, String(result.ResultDesc || 'M-Pesa payment failed')],
        )
      }
