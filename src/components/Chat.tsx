...
function ActionButton({ label, name, onClick }: { label: string; name: string; onClick: () => void }) {
  return <button type="button" aria-label={label} title={label} onClick={onClick} className="flex h-10 w-10 items-center justify-center rounded-full bg-purple-50 transition hover:bg-purple-100 active:scale-95">
...
export default function Chat({ onBack, users = [] }: { onBack: () => void; users: User[] }) {
  ...
  if (active) return <div className="min-h-[70vh] overflow-hidden rounded-[28px] bg-[#FFFBF0] text-neutral-900 shadow-xl ring-1 ring-purple-100 sm:min-h-[76vh] lg:min-h-[78vh]">
    <header className="sticky top-0 z-20 flex min-h-[76px] items-center gap-3 border-b border-purple-100 bg-[#FFFBF0]/95 px-3 py-3 backdrop-blur-xl sm:px-5">
      <button type="button" aria-label="Back" onClick={() => { setActive(null); setGroupInfo(false) }} className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-white shadow-sm ring-1 ring-purple-100 transition hover:bg-purple-50 active:scale-95"><IgIcon name="back" size={21} /></button>
      ...
      <button type="button" onClick={() => setGroupInfo(v => !v)} className="min-w-0 flex-1 text-left">
      ...
      <button type="button" aria-label="Conversation info" onClick={() => setGroupInfo(v => !v)} className="flex h-10 w-10 items-center justify-center rounded-full bg-white ring-1 ring-purple-100"><IgIcon name="settings" size={19} /></button>
    </header>

    {groupInfo && active.username === GROUP.username && <section className="border-b border-purple-100 bg-gradient-to-r from-purple-50 via-white to-amber-50 px-4 py-4 sm:px-6"><div className="mx-auto flex max-w-3xl flex-col gap-4 sm:flex-row sm:items-center"><div className="flex flex-1 items-center gap-3"><Avatar user={GROUP} group size="md" /><div><p className="font-bold">Youth Group</p><p className="text-xs text-stone-500">12 members • invite-only</p></div></div><div className="flex w-full gap-2 sm:max-w-sm"><input value={inviteTarget} onChange={e => setInviteTarget(e.target.value)} placeholder="Username to invite" className="min-w-0 flex-1 rounded-2xl border border-purple-100 bg-white px-3 py-2.5 text-sm outline-none transition focus:border-purple-400 focus:ring-4 focus:ring-purple-100"/><button type="button" onClick={invite} className="rounded-2xl bg-gradient-to-r from-purple-600 to-amber-500 px-4 text-sm font-semibold text-white shadow-md transition hover:shadow-lg active:scale-95 focus:outline-none focus-visible:ring-4 focus-visible:ring-purple-300 focus-visible:ring-offset-2">Invite</button></div></div>{inviteStatus && <p className="mx-auto mt-2 max-w-3xl text-xs text-stone-500">{inviteStatus}</p>}</section>}

    <main className="mx-auto flex w-full max-w-4xl flex-1 flex-col overflow-auto px-3 py-5 sm:px-6 sm:py-7">
      {thread.length === 0 && <div className="flex flex-1 flex-col items-center justify-center py-10 text-center sm:py-12"><div className="rounded-[28px] bg-gradient-to-br from-purple-100 via-amber-50 to-teal-50 p-3"><Avatar user={active} group={active.username === GROUP.username} size="lg" /></div><p className="mt-4 font-bold">{active.username === GROUP.username ? 'Youth Group' : `@${active.username}`}</p><p className="mt-1 max-w-xs text-sm leading-6 text-stone-500">Start a meaningful conversation and keep the community connected. 🙏</p>{active.username !== GROUP.username && <div className="mt-6 w-full max-w-xl"><div className="mb-3 flex items-end justify-between gap-3 text-left"><div><p className="text-[11px] font-bold uppercase tracking-[0.16em] text-purple-600">Need a starting point?</p><p className="mt-1 text-xs text-stone-400">A gentle opener for your first message</p></div><span className="hidden rounded-full bg-white px-2.5 py-1 text-[10px] font-semibold text-stone-400 shadow-sm ring-1 ring-purple-100 sm:inline">Edit before sending</span></div><div className="grid gap-3 sm:grid-cols-3">{QUICK_START_PROMPTS.map(prompt => <button type="button" key={prompt.label} onClick={() => startWithPrompt(prompt.text)} aria-label={`${prompt.label}: ${prompt.hint}`} aria-describedby={`quick-start-${prompt.accent}`} className={`group relative overflow-hidden rounded-[24px] border bg-white p-4 text-left shadow-[0_8px_24px_rgba(91,33,182,0.07)] transition duration-200 hover:-translate-y-1 hover:shadow-[0_14px_30px_rgba(91,33,182,0.12)] active:translate-y-0 active:scale-[0.98] focus:outline-none focus-visible:-translate-y-0.5 focus-visible:ring-4 focus-visible:ring-offset-2 ${prompt.accent === 'amber' ? 'border-amber-200/80 hover:border-amber-300 focus-visible:ring-amber-300/70' : prompt.accent === 'purple' ? 'border-purple-200/80 hover:border-purple-300 focus-visible:ring-purple-400/70' : 'border-teal-200/80 hover:border-teal-300 focus-visible:ring-teal-400/70'}`}><div className={`absolute inset-x-0 top-0 h-1 ${prompt.accent === 'amber' ? 'bg-gradient-to-r from-amber-400 to-amber-500' : prompt.accent === 'purple' ? 'bg-gradient-to-r from-purple-500 to-purple-700' : 'bg-gradient-to-r from-teal-500 to-teal-600'}`} /><div className="flex items-start justify-between gap-2"><span aria-hidden="true" className={`flex h-11 w-11 items-center justify-center rounded-[16px] text-lg shadow-sm ring-1 ${prompt.accent === 'amber' ? 'bg-gradient-to-br from-amber-50 to-orange-100 text-amber-700 ring-amber-200/70' : prompt.accent === 'purple' ? 'bg-gradient-to-br from-purple-50 to-fuchsia-100 text-purple-700 ring-purple-200/70' : 'bg-gradient-to-br from-teal-50 to-cyan-100 text-teal-700 ring-teal-200/70'}`}>{prompt.icon}</span><span aria-hidden="true" className={`rounded-full px-2 py-1 text-[9px] font-bold uppercase tracking-[0.12em] ${prompt.accent === 'amber' ? 'bg-amber-50 text-amber-700' : prompt.accent === 'purple' ? 'bg-purple-50 text-purple-700' : 'bg-teal-50 text-teal-700'}`}>{prompt.eyebrow}</span></div><span className="mt-3 block text-sm font-extrabold tracking-tight text-stone-900">{prompt.label}</span><span id={`quick-start-${prompt.accent}`} className="mt-1 block min-h-[32px] text-[11px] leading-4 text-stone-500">{prompt.hint}</span><span aria-hidden="true" className={`mt-4 inline-flex items-center gap-1.5 text-[10px] font-bold ${prompt.accent === 'amber' ? 'text-amber-700' : prompt.accent === 'purple' ? 'text-purple-700' : 'text-teal-700'}`}>Use prompt <span className="transition-transform group-hover:translate-x-0.5">→</span></span></button>)}</div><p className="mt-3 text-[11px] text-stone-400">Choose a prompt to place it in the message box. You can edit it before sending.</p></div>}</div>}
    ...
    {musicOpen && <section className="border-t border-purple-100 bg-white/95 p-3 backdrop-blur-xl sm:px-5"><div className="mx-auto max-w-4xl"><div className="flex gap-2"><div className="flex flex-1 items-center gap-2 rounded-2xl bg-purple-50 px-3 ring-1 ring-purple-100"><IgIcon name="search" size={18}/><input autoFocus value={musicQuery} onChange={e => searchMusic(e.target.value)} placeholder="Find a worship song…" className="min-w-0 flex-1 bg-transparent py-3 text-sm outline-none"/></div><button type="button" aria-label="Close music" onClick={() => setMusicOpen(false)} className="flex h-11 w-11 items-center justify-center rounded-full bg-stone-100"><IgIcon name="close" size={18}/></button></div><div className="mt-2 max-h-40 space-y-1 overflow-auto">{musicResults.map(m => <button type="button" key={m.id} onClick={() => send(`🎵 ${m.title} • ${m.artist}`, { music: m })} className="flex w-full items-center gap-3 rounded-2xl p-2 text-left transition hover:bg-purple-50"><img src={m.cover} alt="" className="h-10 w-10 rounded-xl object-cover"/><span className="min-w-0 flex-1"><b className="block truncate text-xs">{m.title}</b><small className="block truncate text-stone-500">{m.artist}</small></span><span className="rounded-full bg-purple-100 px-3 py-1 text-xs font-semibold text-purple-700">Send</span></button>)}</div></div></section>}
    {attachOpen && <section className="border-t border-purple-100 bg-white p-3"><div className="mx-auto flex max-w-4xl justify-center gap-2"><button type="button" onClick={() => fileRef.current?.click()} className="flex items-center gap-2 rounded-2xl bg-amber-50 px-4 py-2.5 text-sm font-semibold text-amber-700"><IgIcon name="image" size={18}/> Photo</button><button type="button" onClick={() => fileRef.current?.click()} className="flex items-center gap-2 rounded-2xl bg-purple-50 px-4 py-2.5 text-sm font-semibold text-purple-700"><IgIcon name="video" size={18}/> Video</button><input ref={fileRef} type="file" accept="image/*,video/*" onChange={chooseMedia} className="hidden" /></div></section>}
    {mediaPreview && <section className="border-t border-purple-100 bg-white p-3"><div className="mx-auto flex max-w-4xl items-end gap-3"><div className="relative shrink-0">{mediaType === 'image' ? <img src={mediaPreview} alt="Preview" className="h-20 w-20 rounded-2xl object-cover" /> : <video src={mediaPreview} controls className="h-20 w-20 rounded-2xl object-cover" />}<button type="button" aria-label="Remove media" onClick={() => { setMediaPreview(null); setMediaType(null) }} className="absolute -right-2 -top-2 flex h-6 w-6 items-center justify-center rounded-full bg-neutral-900 text-white"><IgIcon name="close" size={13} active/></button></div><input value={mediaCaption} onChange={e => setMediaCaption(e.target.value)} placeholder="Add a caption…" className="min-w-0 flex-1 rounded-2xl border border-purple-100 bg-purple-50 px-4 py-3 text-sm outline-none focus:border-purple-400 focus:ring-4 focus:ring-purple-100"/><button type="button" aria-label="Send media" onClick={() => send(mediaCaption)} className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-purple-600 to-amber-500 text-white shadow-md"><IgIcon name="send" size={18} active/></button></div></section>}
    {emojiOpen && <section className="border-t border-purple-100 bg-white p-3"><div className="mx-auto grid max-w-4xl grid-cols-7 gap-1 rounded-2xl bg-purple-50 p-2 sm:grid-cols-10">{EMOJIS.map(e => <button type="button" key={e} onClick={() => setText(v => v + e)} className="rounded-xl p-2 text-xl transition hover:bg-white">{e}</button>)}</div></section>}
    <footer className="sticky bottom-0 z-10 border-t border-purple-100 bg-[#FFFBF0]/95 p-3 backdrop-blur-xl sm:p-4"><div className="mx-auto flex max-w-4xl items-center gap-2 rounded-[24px] bg-white p-2 shadow-lg ring-1 ring-purple-100"><button type="button" aria-label="Emoji" onClick={() => setEmojiOpen(v => !v)} className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full transition ${emojiOpen ? 'bg-purple-100' : 'hover:bg-stone-100'}`}><span className="text-lg">☺</span></button><button type="button" aria-label="Add media" onClick={() => setAttachOpen(v => !v)} className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full transition ${attachOpen ? 'bg-amber-100' : 'hover:bg-stone-100'}`}><IgIcon name="plus" size={20}/></button><button type="button" aria-label="Share music" onClick={() => setMusicOpen(v => !v)} className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full transition ${musicOpen ? 'bg-purple-100' : 'hover:bg-stone-100'}`}><IgIcon name="music" size={19}/></button><input ref={composerRef} value={text} onChange={e => handleTyping(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); send() } }} placeholder="Write a message…" className="min-w-0 flex-1 bg-transparent px-1 text-sm outline-none placeholder:text-stone-400"/><button type="button" onClick={() => send()} className="flex h-10 items-center gap-1.5 rounded-full bg-gradient-to-r from-purple-600 to-amber-500 px-4 text-sm font-bold text-white shadow-md transition hover:shadow-lg active:scale-95 focus:outline-none focus-visible:ring-4 focus-visible:ring-purple-300 focus-visible:ring-offset-2"><IgIcon name="send" size={16} active/><span className="hidden sm:inline">Send</span></button></div></footer>
  </div>
  ...
  return <div className="min-h-[70vh] overflow-hidden rounded-[28px] bg-[#FFFBF0] text-neutral-900 shadow-xl ring-1 ring-purple-100 sm:min-h-[76vh]">
    <header className="border-b border-purple-100 bg-gradient-to-r from-purple-50 via-[#FFFBF0] to-amber-50 px-4 pb-4 pt-5 sm:px-6"><div className="flex items-center gap-3"><button type="button" aria-label="Back" onClick={onBack} className="flex h-11 w-11 items-center justify-center rounded-full bg-white shadow-sm ring-1 ring-purple-100"><IgIcon name="back" size={20}/></button><div className="flex-1"><p className="text-xs font-bold uppercase tracking-[0.18em] text-purple-600">Harvest Family</p><h1 className="mt-0.5 text-2xl font-extrabold tracking-tight sm:text-3xl">Messages</h1></div><div className="flex h-11 w-11 items-center justify-center rounded-full bg-gradient-to-br from-purple-600 to-amber-500 text-white shadow-md"><IgIcon name="chat" size={20} active/></div></div><div className="mt-5 flex items-center gap-2 rounded-2xl bg-white px-3 py-2.5 shadow-sm ring-1 ring-purple-100"><IgIcon name="search" size={19}/><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search your community" className="min-w-0 flex-1 bg-transparent text-sm outline-none placeholder:text-stone-400"/></div></header>

    <div className="px-4 pt-4 sm:px-6"><div className="inline-flex rounded-2xl bg-purple-50 p-1"><button type="button" onClick={() => setTab('chats')} className={`rounded-xl px-5 py-2 text-sm font-semibold transition ${tab === 'chats' ? 'bg-white text-purple-700 shadow-sm' : 'text-stone-500'}`}>Chats</button><button type="button" onClick={() => setTab('groups')} className={`rounded-xl px-5 py-2 text-sm font-semibold transition ${tab === 'groups' ? 'bg-white text-purple-700 shadow-sm' : 'text-stone-500'}`}>Groups</button></div></div>

    <div className="px-4 py-5 sm:px-6"><div className="mb-3 flex items-center justify-between"><p className="text-sm font-bold text-stone-700">{tab === 'chats' ? 'People in your community' : 'Community groups'}</p><span className="text-xs text-stone-400">{tab === 'chats' ? people.length : '1 group'}</span></div><div className="flex gap-3 overflow-x-auto pb-1">{tab === 'chats' && people.slice(0, 8).map(u => <button type="button" key={u.username} onClick={() => setActive(u)} className="min-w-[72px] text-center"><div className="mx-auto w-fit"><Avatar user={u} online={!!presence[u.username]?.online} size="md"/></div><p className="mt-1.5 truncate text-[11px] font-semibold text-stone-700">{u.username}</p></button>)}{tab === 'chats' && people.length === 0 && <p className="py-3 text-sm text-stone-500">No community members match your search.</p>}{tab === 'groups' && <button type="button" onClick={() => setActive(GROUP)} className="flex min-w-[230px] items-center gap-3 rounded-[22px] bg-gradient-to-br from-purple-600 to-teal-600 p-3 text-left text-white shadow-lg"><div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-white/15 font-bold">Y</div><div><p className="font-bold">Youth Group</p><p className="mt-0.5 text-xs text-white/75">12 members • invite-only</p></div></button>}</div></div>

    <div className="px-3 pb-5 sm:px-5 sm:pb-7">{tab === 'chats' ? <div className="space-y-2">
      {people.length === 0 && query && <div className="rounded-[24px] bg-gradient-to-br from-white via-purple-50/60 to-amber-50/60 px-6 py-12 text-center shadow-sm ring-1 ring-purple-100"><div className="mx-auto flex h-16 w-16 items-center justify-center rounded-[22px] bg-gradient-to-br from-purple-100 to-teal-50 text-purple-700 shadow-sm ring-1 ring-purple-100"><IgIcon name="search" size={26}/></div><p className="mt-4 text-[11px] font-bold uppercase tracking-[0.16em] text-purple-600">Community search</p><p className="mt-1 text-xl font-extrabold tracking-tight text-stone-900">No one found for “{query}”</p><p className="mx-auto mt-2 max-w-sm text-sm leading-6 text-stone-500">Try a first name, username, or a shorter search. Someone from your Harvest Family community may be just one letter away.</p><div className="mt-5 flex flex-wrap items-center justify-center gap-2"><span className="rounded-full bg-white px-3 py-1.5 text-xs font-semibold text-purple-700 shadow-sm ring-1 ring-purple-100">Try a first name</span><span className="rounded-full bg-white px-3 py-1.5 text-xs font-semibold text-teal-700 shadow-sm ring-1 ring-teal-100">Try @username</span></div><button type="button" onClick={() => setQuery('')} className="mt-5 rounded-full bg-gradient-to-r from-purple-600 to-amber-500 px-5 py-2.5 text-xs font-bold text-white shadow-md transition hover:shadow-lg active:scale-95 focus:outline-none focus-visible:ring-4 focus-visible:ring-purple-300 focus-visible:ring-offset-2">Clear search</button></div>}
