...
const QUICK_START_PROMPTS = [
  { icon: '🙏', label: 'Prayer', text: 'Hi! I would love to pray together. How can I pray for you today?' },
  { icon: '💜', label: 'Encouragement', text: 'Hi! Just wanted to encourage you today. You are not alone. 🙏' },
  { icon: '✨', label: 'Connection', text: 'Hi! I would love to connect and get to know you better.' },
]
...
export default function Chat({ onBack, users = [] }: { onBack: () => void; users: User[] }) {
  ...
  if (active) return <div className="min-h-[70vh] overflow-hidden rounded-[28px] bg-[#FFFBF0] text-neutral-900 shadow-xl ring-1 ring-purple-100 sm:min-h-[76vh] lg:min-h-[78vh]">
    ...
    <main className="mx-auto flex w-full max-w-4xl flex-1 flex-col overflow-auto px-3 py-5 sm:px-6 sm:py-7">
      {thread.length === 0 && <div className="flex flex-1 flex-col items-center justify-center py-10 text-center sm:py-12"><div className="rounded-[28px] bg-gradient-to-br from-purple-100 via-amber-50 to-teal-50 p-3"><Avatar user={active} group={active.username === GROUP.username} size="lg" /></div><p className="mt-4 font-bold">{active.username === GROUP.username ? 'Youth Group' : `@${active.username}`}</p><p className="mt-1 max-w-xs text-sm leading-6 text-stone-500">Start a meaningful conversation and keep the community connected. 🙏</p>{active.username !== GROUP.username && <div className="mt-6 w-full max-w-xl"><p className="text-[11px] font-bold uppercase tracking-[0.16em] text-purple-600">Need a starting point?</p><div className="mt-3 grid gap-2 sm:grid-cols-3">{QUICK_START_PROMPTS.map(prompt => <button key={prompt.label} onClick={() => startWithPrompt(prompt.text)} className="group rounded-[20px] bg-white p-3 text-left shadow-sm ring-1 ring-purple-100 transition hover:-translate-y-0.5 hover:shadow-md hover:ring-purple-200 active:scale-[0.98]"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-purple-50 to-amber-50 text-base ring-1 ring-purple-100">{prompt.icon}</span><span className="mt-2 block text-xs font-bold text-stone-800">{prompt.label}</span><span className="mt-0.5 block text-[11px] leading-4 text-stone-500">Use a gentle opener</span></button>)}</div><p className="mt-3 text-[11px] text-stone-400">Choose a prompt to place it in the message box. You can edit it before sending.</p></div>}</div>}
