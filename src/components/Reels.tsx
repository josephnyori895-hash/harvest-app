...
export default function Reels() {
  ...
  const allVideos = useMemo(() => [...approvedVideos, ...serverReels, ...videosData], [approvedVideos, serverReels])
  const cur: Reel = allVideos[Math.min(idx, allVideos.length - 1)] ?? videosData[0]

  // Load approved reels from the server (signed poster + HLS/source URLs)
  useEffect(() => {
    if (!useServer) return
    let cancelled = false
    fetchReels()
      .then(r => {
        if (cancelled) return
        const mapped = (r.reels || [])
          .filter((r: any) => r.hls_url || r.poster_url)
          .map((r: any) => ({
            id: r.id,
            user: r.username || 'harvest',
            verified: Boolean(r.verified),
            cap: r.caption || 'A Harvest moment',
            views: r.views ?? 0,
            comments: r.comments ?? 0,
            img: r.poster_url || 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=700&h=900&fit=crop',
            video: r.hls_url || undefined,
            src: 'server' as const,
          }))
        setServerReels(mapped)
      })
      .catch(() => { /* offline — demo + approved local reels still play */ })
  ...
  // Reset to a valid index if the list shrinks (e.g. server reels arrive late)
  useEffect(() => { if (idx >= allVideos.length && allVideos.length > 0) setIdx(0) }, [idx, allVideos.length])
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-6 py-5">
        ...
        <div className="grid lg:grid-cols-[minmax(0,760px)_260px] gap-5 items-stretch">
          <section className="relative overflow-hidden rounded-[28px] bg-black min-h-[600px] lg:h-[calc(100vh-190px)] lg:max-h-[760px] border border-white/10 shadow-2xl">
            {cur.video ? <video key={cur.video} ref={videoRef} src={cur.video} autoPlay muted={muted} loop playsInline poster={cur.img} className="absolute inset-0 w-full h-full object-cover" /> : <img src={cur.img} alt="" className="absolute inset-0 w-full h-full object-cover" />}
            ...
            <div className="absolute left-5 right-20 bottom-6">
              <div className="flex items-center gap-3 mb-3">
                <div className="w-11 h-11 rounded-full bg-gradient-to-br from-amber-300 to-purple-500 flex items-center justify-center font-bold">{String(cur.user).charAt(0).toUpperCase()}</div>
              ...
              <div className="mt-3 flex gap-4 text-xs text-white/60"><span>👀 {fmtViews(cur.views)}</span><span>💬 {fmtViews(cur.comments)} responses</span></div>
            </div>
          ...
          <aside className="hidden lg:block rounded-[24px] bg-[#2c2722] border border-white/10 p-4">
            ...
            {/* Quick list of the actual reels — tap to jump */}
