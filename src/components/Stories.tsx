...
export default function StoryViewer({ idx, setIdx, allStories, users = [], onOpenUser, onDeleted, onViewed }: { idx: number; setIdx: (n: number | null) => void; allStories: any[]; users?: any[]; onOpenUser?: (u: any) => void; onDeleted?: (id: string) => void; onViewed?: (id: string) => void }) {
  ...
  const isStoryAuthor = Boolean(s?.username && s.username === me)
  const deleteReply = async (replyId: string) => {
    if (!replyId) return
    try {
      const API=(import.meta.env.VITE_API_URL||'').replace(/\/$/,'')
      const t=localStorage.getItem('harvest_token')||''
      const r=await fetch(API+'/api/stories/'+encodeURIComponent(String(s.id))+'/replies?id='+encodeURIComponent(replyId),{method:'DELETE',headers:t?{Authorization:'Bearer '+t}:undefined})
  ...
  return (
    <>
      ...
      <div className="flex-1 flex items-center justify-center relative overflow-hidden">
        ...
        {s.img && !isVideo && videoState === 'error' && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/60">
            <p className="text-sm text-stone-300">Story image could not load</p>
        ...
        {isVideo && (
          <>
            ...
            {videoState === 'error' && (
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/60">
                <p className="text-sm text-stone-300">Video could not load</p>
                <p className="text-[11px] text-stone-500 mt-1">Check your connection and try again</p>
                <button type="button" onClick={e => { e.stopPropagation(); setVideoState('loading'); setMediaRetryKey(k => k + 1) }} className="mt-3 min-h-11 px-4 rounded-xl bg-white/10 border border-white/15 text-xs font-semibold">Try again</button>
              </div>
            )}
          </>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6"><p className="font-bold text-white text-lg">{s.name}</p><p className="text-sm text-stone-300 mt-1">{s.caption || 'Harvest story 🙏'}</p>{s.music && <div className="flex gap-2 items-center mt-2 p-2 bg-black/60 rounded-lg"><img src={s.music.cover} className="w-8 h-8 rounded" /><div className="flex-1"><p className="text-xs font-semibold">🎵 {s.music.title}</p><p className="text-[11px] text-stone-400">{s.music.artist}</p></div><a href={s.music.url} target="_blank" rel="noreferrer" className="text-xs bg-white text-black px-2 py-1 rounded-full">▶</a></div>}</div>
      ...
      {showReplies && <div onClick={e=>e.stopPropagation()} className="absolute left-3 right-3 max-h-40 overflow-auto rounded-2xl bg-black/80 p-3 text-white" style={{ bottom: keyboardInset ? `${keyboardInset + 64}px` : "calc(var(--safe-area-inset-bottom, env(safe-area-inset-bottom)) + 6rem)" }}><div className="flex justify-between mb-2"><b className="text-xs">Replies</b><button onClick={()=>setShowReplies(false)}>×</button></div>{replies.length ? replies.map((r:any)=>{
            const canRemove = isAdmin || isStoryAuthor || (r.username && r.username === me)
            return <div key={r.id} className="flex items-start gap-2 text-xs py-1"><p className="min-w-0 flex-1"><b>{r.name||r.username}</b> {r.body}</p>{canRemove && <button type="button" onClick={()=>void deleteReply(String(r.id))} className="min-w-11 min-h-11 shrink-0 text-stone-300 hover:text-white" aria-label="Delete reply">×</button>}</div>
          }) : <p className="text-xs text-stone-400">No replies yet.</p>}</div>}
      <div onClick={e=>e.stopPropagation()} className="absolute left-3 right-3 flex gap-2" style={{ bottom: keyboardInset ? `${keyboardInset + 12}px` : "calc(var(--safe-area-inset-bottom, env(safe-area-inset-bottom)) + 3rem)" }}><input value={reply} onChange={e=>setReply(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')void sendReply()}} placeholder="Reply to this story…" className="flex-1 min-h-11 rounded-full bg-white/95 text-black px-4 py-2 text-sm outline-none focus:ring-2 focus:ring-purple-400" /><button onClick={()=>void sendReply()} disabled={replyBusy} className="min-h-11 rounded-full bg-purple-600 text-white px-4 py-2 text-xs font-bold">{replyBusy?'…':'Send'}</button><button onClick={()=>{setShowReplies(v=>!v); if(!showReplies)void loadReplies()}} className="min-w-11 min-h-11 rounded-full bg-white/20 text-white px-3 py-2 text-xs">💬 {replies.length||''}</button></div>
...
export function StoryCreate({ onDone }: { onDone: () => void }) {
  ...
  return (
    <div className="bg-stone-50 text-white min-h-[calc(100vh-49px)] flex flex-col">
      <div className="flex justify-between items-center px-4 h-[56px] border-b border-stone-200 bg-white"><button onClick={onDone} className="text-xl text-stone-700">✕</button><p className="font-semibold text-sm text-stone-900">Create Story</p><button onClick={() => void submit()} disabled={busy} className={`font-semibold text-sm ${busy ? 'text-stone-400' : 'text-[#7C3AED]'}`}>{busy ? 'Sharing…' : 'Share'}</button></div>
      <div className="flex-1 flex items-center justify-center p-4 relative bg-gradient-to-b from-stone-50 to-white">{!fileUrl ? <label className="w-full aspect-[9/16] bg-white rounded-2xl border-2 border-dashed border-stone-300 flex flex-col items-center justify-center cursor-pointer hover:border-[#7C3AED] transition"><input ref={fileInputRef} type="file" accept="image/*,video/*" onChange={onFile} className="hidden" /><div className="w-20 h-20 rounded-full bg-[#7C3AED] flex items-center justify-center text-4xl text-white">📷</div><p className="text-sm text-stone-600 mt-2">Tap to add story photo/video</p><p className="text-xs text-stone-400 mt-1">Record with camera or pick from gallery</p></label> : <div className="relative aspect-[9/16] w-full max-w-[320px] mx-auto overflow-hidden rounded-2xl border border-stone-200 shadow-lg bg-stone-900">{isVideoFile
      ...
      {fileUrl && !isVideoFile && <><div className="px-4 pb-2 bg-white border-t border-stone-200"><button onClick={() => { setShowFilters(!showFilters); setShowStickers(false); setShowMusic(false) }} className="text-xs text-[#7C3AED] font-semibold">🎨 Filter: {filter}</button>{showFilters && <div className="flex gap-2 overflow-x-auto pb-2 mt-1">{FILTERS.map(f => <button key={f} onClick={() => { setFilter(f); applyFilter(fileUrl, f); setShowFilters(false) }} className={`px-3 py-1 rounded-full text-xs whitespace-nowrap ${filter === f ? 'bg-[#7C3AED] text-white' : 'bg-stone-100 text-stone-700'}`}>{f}</button>)}</div>}</div><div className="px-4 pb-2 bg-white border-t border-stone-200"><button onClick={() => { setShowStickers(!showStickers); setShowFilters(false); setShowMusic(false) }} className="text-xs text-[#7C3AED] font-semibold">📷 Stickers: {stickers.length > 0 ? stickers.join(' ') : 'none'}</button>{showStickers && <div className="flex gap-2 overflow-x-auto pb-2 mt-1 flex-wrap">{STORY_STICKERS.map(s => <button key={s} onClick={() => stickers.includes(s) ? removeSticker(s) : addSticker(s)} className={`text-xl px-2 py-1 rounded-full ${stickers.includes(s) ? 'bg-[#7C3AED] text-white' : 'bg-stone-100 text-stone-700'}`}>{s}</button>)}</div>}</div><div className="px-4 pb-2 bg-white border-t border-stone-200 rounded-xl p-2"><button onClick={() => { setShowMusic(!showMusic); setShowMood(!showMood); setShowFilters(false); setShowStickers(false) }} className="text-xs text-[#7C3AED] font-semibold">🎵 Music {musicTrack ? `• ${musicTrack.title} ✓` : ''}</button>{musicTrack && <div className="flex gap-2 items-center mt-2 p-2 bg-stone-50 rounded-lg"><img src={musicTrack.cover} className="w-10 h-10 rounded"/><div className="flex-1 min-w-0"><p className="text-xs font-semibold truncate">{musicTrack.title}</p><p className="text-[11px] text-stone-400 truncate">{musicTrack.artist}</p></div><button onClick={() => togglePreview(musicTrack)} className="w-7 h-7 rounded-full bg-white text-black flex items-center justify-center text-xs">{previewId === musicTrack.id ? '⏸' : '▶'}</button><button onClick={() => setMusicTrack(null)} className="text-xs text-red-400">✕</button></div>}{showMood && <div className="flex gap-2 flex-wrap mt-2">{MOOD_PICKS.map(m => <button key={m.mood} onClick={() => filterByMood(m.tags)} className="px-3 py-2 rounded-full bg-purple-100 text-purple-700 text-xs font-bold">{m.mood}</button>)}</div>}{showMusic && !showMood && <div className="mt-2"><div className="flex gap-2"><input value={musicQuery} onChange={e => { setMusicQuery(e.target.value); searchMusic(e.target.value) }} onKeyDown={e => e.key === 'Enter' && searchMusic(musicQuery)} placeholder="Search Hillsong/Maverick..." className="flex-1 bg-stone-50 border border-stone-200 rounded-full px-3 py-2 text-xs outline-none" /><button onClick={() => searchMusic(musicQuery)} className="px-3 py-2 rounded-full bg-[#7C3AED] text-white text-xs font-bold">Search</button></div>{musicLoading && <p className="text-xs text-stone-500 text-center py-2">Searching…</p>}{!musicLoading && musicResults.length === 0 && musicQuery && <p className="text-xs text-stone-500 text-center py-2">No results — try Hillsong/Maverick/Sinach</p>}{!musicLoading && musicResults.length === 0 && !musicQuery && <div className="flex gap-2 flex-wrap mt-2">{['Hillsong', 'Maverick', 'Sinach', 'Elevation', 'Harvest'].map(t => <button key={t} onClick={() => { setMusicQuery(t); searchMusic(t) }} className="px-3 py-1 rounded-full bg-stone-100 text-xs">{t}</button>)}</div>}{musicResults.map((m: any) => <div key={m.id} className="flex gap-2 p-2 bg-stone-50 rounded-lg items-center"><img src={m.cover} className="w-10 h-10 rounded"/><div className="flex-1 min-w-0"><p className="text-xs font-semibold truncate">{m.title}</p><p className="text-[11px] text-stone-400 truncate">{m.artist}</p></div><button onClick={() => togglePreview(m)} className="w-7 h-7 rounded-full bg-stone-200 flex items-center justify-center text-xs">{previewId === m.id ? '⏸' : '▶'}</button><button onClick={() => chooseMusic(m)} className="px-3 py-1 rounded-full bg-[#7C3AED] text-white text-xs">Use</button></div>)}</div>}</div><div className="px-4 pb-2 bg-white border-t border-stone-200"><p className="text-xs text-[#7C3AED] mb-1 font-semibold">✏️ Text overlay</p><input value={textOverlay} onChange={e => setTextOverlay(e.target.value)} placeholder="Add bold text..." className="w-full bg-stone-50 border border-stone-200 rounded-xl px-3 py-2 text-sm outline-none" /><div className="flex gap-2 mt-1 items-center"><span className="text-xs text-stone-400">Color:</span>{['#ffffff', '#7C3AED', '#ed4956', '#f77737', '#40d657', '#b546cf', '#000000'].map(c => <button key={c} onClick={() => setTextColor(c)} className="w-6 h-6 rounded-full border-2 border-white shadow-sm" style={{ backgroundColor: c }} />)}<button onClick={() => setTextBold(!textBold)} className={`ml-2 px-2 py-1 rounded-full text-xs font-bold ${textBold ? 'bg-[#7C3AED] text-white' : 'bg-stone-200 text-stone-600'}`}>B:{textBold?'on':'off'}</button></div></div><div className="px-4 pb-2 bg-white border-t border-stone-200"><p className="text-xs text-[#7C3AED] mb-1 font-semibold">⏱️ Story timer</p><div className="flex gap-2">{[0, 3, 5, 10, 15].map(s => <button key={s} onClick={() => setTimer(s)} className={`px-3 py-1 rounded-full text-xs ${timer === s ? 'bg-[#7C3AED] text-white' : 'bg-stone-100 text-stone-700'}`}>{s === 0 ? 'Off' : `${s}s`}</button>)}</div></div></>}
      {notice && <div className="mx-4 mb-2"><ErrorMessage message={notice} /></div>}
      {adjusting && fileObj && fileObj.type.startsWith('image/') && (
        <ImageAdjuster file={fileObj} onCancel={() => setAdjusting(false)} onDone={applyAdjust} />
      )}
      <div className="px-4 pb-4 bg-white border-t border-stone-200"><input value={caption} onChange={e => setCaption(e.target.value)} placeholder="Write a caption..." className="w-full bg-stone-50 border border-stone-200 rounded-xl px-4 py-3 text-sm outline-none" /></div>
    </div>
  )
}
