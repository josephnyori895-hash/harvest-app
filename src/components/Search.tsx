    ...
    <div className="bg-black text-white min-h-[70vh] p-2">
      ...
      {!showGrid ? (
        ...
        <>
          ...
          {recent.length > 0 && (
            <>
              <p className="text-xs text-zinc-500 px-1 mt-4">Recent from the community</p>
              <div className="grid grid-cols-3 gap-[2px] mt-2">
                {recent.map((p, i) => (
                  <button key={p.id || i} onClick={() => onView({ username: p.username, name: p.name })} className="aspect-square bg-zinc-900 overflow-hidden relative group">
                    ...
                    {p.kind === 'reel' && <span className="absolute top-1 right-1 text-xs">▶</span>}
                  </button>
                ))}
              </div>
            </>
