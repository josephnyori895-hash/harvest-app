  ...
  return (
    <div className="bg-black text-white min-h-[70vh] p-4">
      ...
      <div className="mt-6 border-t border-zinc-800 pt-4">
        ...
        <div className="mt-3 space-y-2">{users.map(u => <div key={u.username} className="flex items-center justify-between p-3 rounded-xl bg-zinc-900 border border-zinc-800"><div className="flex gap-3 items-center"><div className="w-10 h-10 rounded-full bg-zinc-700 flex items-center justify-center text-sm">{u.username[0].toUpperCase()}</div><div><p className="text-sm font-semibold flex items-center gap-1">{u.username} {u.verified && <span className="w-3 h-3 rounded-full bg-blue-500 flex items-center justify-center text-[8px]">✓</span>}</p><p className="text-xs text-zinc-400">{u.name} • {u.group}</p></div></div><button onClick={() => toggleVerify(u.username)} className={`px-4 py-1.5 rounded-full text-xs font-semibold ${u.verified ? 'bg-blue-500 text-white' : 'bg-white text-black'}`}>{u.verified ? 'Unverify' : 'Verify'}</button></div>)}</div>
      </div>
