    ...
    <div className="bg-black text-white min-h-[70vh] p-4">
      ...
      {pending.length === 0 ? <p className="text-sm text-zinc-500 text-center py-4">No pending posts — all caught up ✓</p> : pending.map(item => (
        <div key={item.id} className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 mb-3">
          ...
          <p className="text-sm font-semibold mt-2">{item.user}</p><p className="text-sm text-zinc-300 mt-1">{item.caption || 'No caption'}</p>
          {item.img && <img src={item.img} alt="" className="w-full h-24 object-cover rounded-lg mt-2" />}
          <div className="flex gap-2 mt-3"><button onClick={() => onApprove(item.id)} className="flex-1 py-2 rounded-full bg-green-600 text-white text-sm font-semibold">✓ Approve</button><button onClick={() => onReject(item.id)} className="flex-1 py-2 rounded-full bg-zinc-800 text-white text-sm font-semibold">✕ Reject</button></div>
        </div>
      ))}
        ...
        <div className="mt-3 space-y-2">
          {users.map(u => (
            <div key={u.username} className="flex items-center justify-between p-3 rounded-xl bg-zinc-900 border border-zinc-800">
              <div className="flex gap-3 items-center"><div className="w-10 h-10 rounded-full bg-zinc-700 flex items-center justify-center text-sm">{u.username[0].toUpperCase()}</div><div><p className="text-sm font-semibold flex items-center gap-1">{u.username} {u.verified && <span className="w-3 h-3 rounded-full bg-blue-500 flex items-center justify-center text-[8px]">✓</span>}</p><p className="text-xs text-zinc-400">{u.name} • {u.group}</p></div></div>
              <button disabled={busy === u.username} onClick={() => void toggleVerify(u.username)} className={`px-4 py-1.5 rounded-full text-xs font-semibold disabled:opacity-50 ${u.verified ? 'bg-blue-500 text-white' : 'bg-white text-black'}`}>{busy === u.username ? 'Saving…' : u.verified ? 'Unverify' : 'Verify'}</button>
            </div>
          ))}
        </div>
      </div>
