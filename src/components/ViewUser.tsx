  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-amber-50 to-purple-50 text-neutral-900">
      ...
      <div className="mt-6 px-4">
        ...
        {posts.length === 0 ? <div className="text-center py-12"><div className="text-5xl mb-3">📸</div><p className="text-neutral-600 font-medium">{loaded ? 'No posts yet' : 'Loading…'}</p></div> : <div className="grid grid-cols-3 gap-1 bg-neutral-200 rounded-xl overflow-hidden">{posts.map((p: any, i: number) => <button key={p.id || i} onClick={() => setPostIdx(i)} className="aspect-square overflow-hidden hover:opacity-80 transition-opacity group relative">{p.hls_url ? <><video src={p.hls_url} className="w-full h-full object-cover" poster={p.thumb_url} muted /><div className="absolute inset-0 bg-black/20 group-hover:bg-black/40 transition-all flex items-center justify-center"><span className="text-white text-2xl">▶</span></div></> : p.thumb_url ? <img src={p.thumb_url} alt="" className="w-full h-full object-cover" /> : <div className="w-full h-full bg-neutral-100 flex items-center justify-center text-2xl">🙏</div>}</button>)}</div>}
      </div>
