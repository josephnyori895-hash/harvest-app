...
export default function Home({ setTab, users, onDeleteStory, refreshKey, onSwitchAccount }: { setTab: (t: string) => void; users: any[]; onDeleteStory?: (id: string) => void; refreshKey?: number; onSwitchAccount?: ()=>void }) {
  ...
  // listen for storage + custom harvest event so Admin approve refreshes instantly
  useEffect(() => {
  ...
  const isVerified = (username: string, fallback?: boolean) => {
    if (username in verifiedMap) return verifiedMap[username]
    return !!fallback
  }
...
<div className="flex items-center justify-between px-4 h-[60px] border-b border-zinc-800 sticky top-0 bg-black z-10">
         <button onClick={()=>onSwitchAccount&&onSwitchAccount()} className="flex items-center gap-1"><h1 className="text-[24px] font-normal" style={{ fontFamily: 'cursive' }}>Harvest</h1><span className="text-xs">⌄</span></button>
         <div className="flex gap-4 items-center">
           <button onClick={() => setTab('activity')} aria-label="activity"><svg width="23" height="23" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="1.7"><path d="M12 21s-6-4-6-10a6 6 0 1 1 12 0c0 6-6 10-6 10z" /></svg></button>
           <button onClick={() => setTab('chat')} aria-label="chat"><svg width="23" height="23" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="1.6"><path d="M22 2L11 13M22 2l-7 20-4-9-9-4 20-7z" /></svg></button>
           {isAdmin&&<button onClick={clearCache} className="text-[10px] bg-zinc-800 text-zinc-400 px-2 py-1 rounded-full" title="Clear cache">🗑️</button>}
         </div>
       </div>

      <div className="flex gap-3 px-3 py-3 overflow-x-auto scrollbar-none border-b border-zinc-800">
...
const isOwn = !s.me && (s.name===currentUser || String(s.id).startsWith(currentUser+'_'))
          ...
          <div key={s.name + i} className="flex flex-col items-center min-w-[66px] relative">
            <button onClick={() => s.me ? setTab('post') : setStoryIdx(i)} className="flex flex-col items-center">
              <div className={`w-[66px] h-[66px] rounded-full p-[2px] ${s.me ? 'bg-zinc-700' : 'bg-gradient-to-tr from-yellow-400 via-red-500 to-purple-600'}`}>
                <div className="w-full h-full rounded-full bg-black border-[2px] border-black flex items-center justify-center overflow-hidden text-xs font-bold">{s.me ? '＋' : s.name.slice(0, 2)}</div>
              </div>
              <p className="text-[11px] mt-1 text-white truncate w-[66px] text-center">{s.name}</p>
            </button>
{/* Delete button: only for stories with real approvedStory ids */}
            {onDeleteStory && (()=>{ const hasRealId = approvedStories.some((x:any)=>x.id === s.id); return hasRealId || s.me })() && (
              <button onClick={(e)=>{ e.stopPropagation(); if(s.me){ const approved = JSON.parse(localStorage.getItem('harvest_approved_stories')||'[]'); const filtered = approved.filter((x:any)=>x.id !== s.id); localStorage.setItem('harvest_approved_stories', JSON.stringify(filtered)); window.dispatchEvent(new Event('harvest:approved')); alert('Story deleted ✓') } else if(confirm(`Delete story ${s.name}?`)) onDeleteStory(s.id) }} className="absolute -top-1 -right-1 w-5 h-5 bg-red-600 rounded-full flex items-center justify-center text-[10px] text-white">✕</button>
            )}
          </div>
          )
        })}
      ...
      {allPosts.map((p: any, i: number) => {
        ...
        return (
          <div key={p.user + i} className="border-b border-zinc-800">
<div className="flex items-center justify-between px-3 py-3">
               <div className="flex items-center gap-3"><div className="w-8 h-8 rounded-full bg-gradient-to-tr from-yellow-400 to-purple-600 p-[2px]"><div className="w-full h-full rounded-full bg-black flex items-center justify-center text-[10px] font-bold border border-black">{p.user[0].toUpperCase()}</div></div><div><div className="flex items-center gap-1"><span className="text-[13px] font-semibold">{p.user}</span>{isVerified(p.user, p.verified) && <span className="w-3 h-3 rounded-full bg-[#0095f6] flex items-center justify-center text-[8px] text-white">✓</span>}<span className="text-zinc-500 text-[13px]">• {p.time}</span></div><p className="text-[12px] text-white -mt-0.5">{p.loc}</p></div></div>
               <div className="relative">
                 <button onClick={(e)=>{e.stopPropagation();setMenuPost(menuPost===key?null:key)}} className="text-white text-lg px-1">⋯</button>
                 {menuPost===key && isAdmin && (
                   <div className="absolute right-0 top-6 bg-zinc-900 border border-zinc-700 rounded-xl p-2 z-20 min-w-[140px] shadow-xl">
                     <button onClick={(e)=>{e.stopPropagation();alert('Edit post feature — admin can update caption/media')}} className="block w-full text-left px-3 py-2 text-sm rounded-lg hover:bg-zinc-800">✏️ Edit</button>
                     <button onClick={(e)=>{e.stopPropagation();if(confirm('Delete this post permanently?')){localStorage.setItem('harvest_pending',JSON.stringify((JSON.parse(localStorage.getItem('harvest_pending')||'[]')).filter(x=>x.id!==parseInt(key.split('_').pop()||'0'))));alert('Post deleted')}setMenuPost(null)}} className="block w-full text-left px-3 py-2 text-sm rounded-lg hover:bg-zinc-800 text-red-400">🗑️ Delete</button>
                     <button onClick={(e)=>{e.stopPropagation();alert('Manage post visibility')}} className="block w-full text-left px-3 py-2 text-sm rounded-lg hover:bg-zinc-800">📋 Manage</button>
                   </div>
                 )}
               </div>
             </div>
            {p.video ? <video src={p.video} controls playsInline className="w-full aspect-square object-cover bg-zinc-900" poster={p.img} /> : <img src={p.img} alt="" className="w-full aspect-square object-cover bg-zinc-900" />}
            <div className="px-3 pt-3 pb-2">
              <div className="flex justify-between"><div className="flex gap-4"><button onClick={() => toggleLike(key)} aria-label="like"><svg width="24" height="24" viewBox="0 0 24 24" fill={liked ? '#ed4956' : 'none'} stroke={liked ? '#ed4956' : 'white'} strokeWidth="1.7"><path d="M12 21s-6-4-6-10a6 6 0 0 1 11 .5A6 6 0 0 1 18 11c0 6-6 10-6 10z" /></svg></button><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="1.7"><path d="M22 2L11 13M22 2l-7 20-4-9-9-4 20-7z" /></svg></div><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="1.7"><path d="M19 21l-7-3-7 3V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z" /></svg></div>
              <p className="text-[14px] font-semibold mt-3">{displayLikes.toLocaleString()} likes</p>
              <p className="text-[14px] leading-[18px] mt-1"><span className="font-semibold">{p.user}</span> {p.caption}</p>
              {p.music && <div className="flex gap-2 items-center mt-2 p-2 bg-zinc-900 rounded-lg border border-zinc-800"><img src={p.music.cover} alt="" className="w-8 h-8 rounded"/><div className="flex-1 min-w-0"><p className="text-xs font-semibold truncate">🎵 {p.music.title}</p><p className="text-[11px] text-zinc-400 truncate">{p.music.artist}</p></div><a href={p.music.url} target="_blank" className="text-xs bg-white text-black px-2 py-1 rounded-full">▶ Preview</a></div>}
            </div>
          </div>
        )
      })}
