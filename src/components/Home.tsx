...
function MediaPreview({ src, poster, alt = '' }: { src: string; poster?: string; alt?: string }) {
  ...
  return <div className="relative w-full aspect-[4/3] overflow-hidden bg-[#F4E8D0]">
    ...
    <video src={src} poster={poster || undefined} controls playsInline preload="metadata" className={`absolute inset-0 w-full h-full object-cover bg-[#1a1714] shadow-inner transition-opacity duration-200 ${ready || !poster ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} aria-label="Video" />
