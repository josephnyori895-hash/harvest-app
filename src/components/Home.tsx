...
export default function Home({ setTab, users, onDeleteStory, refreshKey, onSwitchAccount }: { setTab: (t: string) => void; users: any[]; onDeleteStory?: (id: string) => void; refreshKey?: number; onSwitchAccount?: () => void }) {
  ...
  useEffect(() => {
    ...
    setRefreshing(true); setApprovedTick(x => x + 1); setLikesTick(x => x + 1)
    containerRef.current?.scrollTo({ top: 0, behavior: 'smooth' })
    showToast('Community refreshed', 'success', 1200)
    const t = setTimeout(() => setRefreshing(false), 700); return () => clearTimeout(t)
  }, [refreshKey])

  const allMoments = useMemo(() => [...momentsBase, ...approvedMoments.map((s: any) => ({ name: s.name, id: s.id, label: 'Story' }))], [approvedMoments])
  const allUpdates = useMemo(() => [...approvedPosts, ...updatesBase], [approvedPosts])
  ...
  const onTouchEnd = (e: any) => {
    ...
    if (dy > 80 && containerRef.current && containerRef.current.scrollTop < 5 && !refreshing) { setRefreshing(true); setApprovedTick(x => x + 1); setLikesTick(x => x + 1); showToast('Community refreshed', 'success', 1200); setTimeout(() => setRefreshing(false), 700) }
  }
